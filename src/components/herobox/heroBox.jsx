    ...
    >
      <h1 className="f1 f-subheadline-l fw8 tc-l tracked ma0">
        {siteTitle.split("").map((c, i) => {
          ...
          const alphabetical = (ascii > 64 && ascii < 91) || (ascii > 96 && ascii < 123)
          return alphabetical ? <span key={i}>{c}</span> : <span key={i} className="light-red">{c}</span>
        })}
      </h1>
      ...
      {[...Array(Math.ceil(Math.min(links.length, socialEnums.length) / 3))]
        .map((_, i) => {
          return (
            <SocialList key={socialEnums[i]} links={links.slice(i * 3, (i + 1) * 3)} socialEnums={socialEnums.slice(i * 3, (i + 1) * 3)} />
          )
        })}
    </div>
  )

  ...
  return (
    ...
    >
      <h1 className="f1 f-subheadline-l fw8 tc-l tracked ma0">
        {siteTitle.split("").map(c => {
          const ascii = c.charCodeAt(0)
          const alphabetical = (ascii > 64 && ascii < 91) || (ascii > 96 && ascii < 123)
          return alphabetical ? <span>{c}</span> : <span className="light-red">{c}</span>
        })}
      </h1>
      ...
      {[...Array(Math.ceil(Math.min(links.length, socialEnums.length) / 3))]
        .map((_, i) => {
          return (
            <SocialList links={links.slice(i * 3, (i + 1) * 3)} socialEnums={socialEnums.slice(i * 3, (i + 1) * 3)} />
          )
        })}
    </div>
  )
